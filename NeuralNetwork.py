...
#Online resource says: epochs <= 100 is optimal in most cases when batch sizes usually <= 32.
...
hidden_size_1 = 24 #Number of neurons in first hidden layer
hidden_size_2 = 18 #Number of neurons in second hidden layer
hidden_size_3 = 12 #Number of neurons in third hidden layer
hidden_size_4 = 8 #Number of neurons in fourth hidden layer
hidden_size_5 = 4 #Number of neurons in fifth hidden layer
num_classes = 2 #Number of output classes
num_epochs = 96 #Number of epochs
batch_size = 19 #Batch size for training
learning_rate = 0.001 #Learning rate for weight updation

#FINDING & SPLITTING DATASET
features = np.loadtxt("./ergofeatures.txt", dtype='int32', delimiter=',')
label = np.loadtxt("./ergolabels.txt", dtype='int32', delimiter=',')
...
class NeuralNet(nn.Module):
	def __init__(self, input_size, hidden_size_1, hidden_size_2, hidden_size_3, hidden_size_4, hidden_size_5, num_classes):
		...
		self.l2 = nn.Linear(hidden_size_1, hidden_size_2).cuda() #This takes outputs from hidden layer-1 to hidden layer-2
		self.relu = nn.ReLU().cuda() #This performs activation over the weighted sum and bias addition of second hidden layer
		self.l3 = nn.Linear(hidden_size_2, hidden_size_3).cuda() #This takes outputs from hidden layer-2 to hidden layer-3
		self.relu = nn.ReLU().cuda() #This performs activation over the weighted sum and bias addition of second hidden layer
		self.l4 = nn.Linear(hidden_size_3, hidden_size_4).cuda() #This takes outputs from hidden layer-3 to hidden layer-4 
		self.relu = nn.ReLU().cuda() #This performs activation over the weighted sum and bias addition of third hidden layer
		self.l5 = nn.Linear(hidden_size_4, hidden_size_5).cuda() #This takes outputs from hidden layer-4 to hidden layer-5 
		self.relu = nn.ReLU().cuda() #This performs activation over the weighted sum and bias addition of fourth hidden layer
		self.l6 = nn.Linear(hidden_size_5, num_classes).cuda() #This is the output layer, we use cross entropy loss later so no softmax for this (Q).
...
model = NeuralNet(input_size, hidden_size_1, hidden_size_2, hidden_size_3, hidden_size_4, hidden_size_5, num_classes)
